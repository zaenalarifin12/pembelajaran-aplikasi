...
class _DetailGameState extends State<DetailGame> {
  ...
  getDetail() async {
    final response = await http.get(BaseUrl.showGame(widget.idGame));

    if (response.statusCode == 200) {
      final data = jsonDecode(response.body)["data"]["soal"];
      final detail = jsonDecode(response.body)["data"]["game_jawaban"];
      ...
      for (Map i in detail) {
        listDetailGame.add(GameJawabanModel.fromJson(i));
      }
